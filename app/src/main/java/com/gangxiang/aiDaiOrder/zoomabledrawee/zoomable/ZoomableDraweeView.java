...
package com.gangxiang.aiDaiOrder.zoomabledrawee.zoomable;
...
public class ZoomableDraweeView extends DraweeView<GenericDraweeHierarchy>
  ...
  public void resetScale(){
    ...
    mZoomableController.resetScale();
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (mZoomableController.onTouchEvent(event)) {
      ...
      FLog.v(TAG, "onTouchEvent : view %f", mZoomableController.getScaleFactor());
      FLog.v(TAG, "onTouchEvent : view  mImageBounds = %f", mImageBounds.right*mZoomableController.getScaleFactor());


      if( Math.abs(m[Matrix.MTRANS_X]) < 2.0f || Math.abs(m[Matrix.MTRANS_X] + (mImageBounds.right*mZoomableController.getScaleFactor()  -getWidth()))<2.0f ){//////证明已经在边界了。
...
//      }
  ...
  public void onTransformChanged(Matrix transform) {
    ...
    transform.getValues(m);
    FLog.v(TAG, "onTransformChanged: view %x", this.hashCode());
    FLog.v(TAG, "onTransformChanged: Matrix  X= %f", m[Matrix.MTRANS_X]);
    FLog.v(TAG, "onTransformChanged: Matrix  Y= %f", m[Matrix.MTRANS_Y]);
